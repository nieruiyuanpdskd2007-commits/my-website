...
V0.1 deliberately has no password database.  The desktop app runs as a local
...
class LocalGuestAuth:
    """Safe V0.1 provider: no credentials are collected or persisted."""

    def current_session(self) -> UserSession:
        return UserSession("local-guest", "本地访客", False)

    def sign_in(self, email: str, password: str) -> UserSession:
        raise NotImplementedError("登录服务尚未启用；V0.1 不会在本地保存密码。")
